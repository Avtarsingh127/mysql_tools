...
#################################################Redhat#########################################################
    # sudo rpm -i mysql-utilities-1.6.4-el6.noarch.rpm
##########################################################################################################
...
# changed sql mode temporarily. To get rid of error for column which has 0 as default date.
...
mysqlfrm -q --user=root --server=$username:$password@localhost:$myport --port=$mysqlfrmport $restoredir |
    ...
    sed s/^$/';set SQL_MODE=ALLOW_INVALID_DATES;'/ | sed s/^'CREATE TABLE `'$backupdb/'CREATE TABLE `'$database/ |
